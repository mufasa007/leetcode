...
//        int[][] edges = new int[][]{{3,0},{3,1},{3,2},{3,4},{5,4}};
    ...
    public List<Integer> findMinHeightTrees(int n, int[][] edges) {
        ...
        for (int i = 0; i < n; i++) {
            for (int j = 0; j < n; j++) {
                if(i!=j){
                    cost[i][j] = Integer.MAX_VALUE/2-10;
                }
            }
        }
        ...
        for (int i = 0; i < n; i++) {
            for (int j = 0; j < n; j++) {
                for (int k = 0; k < n; k++) {
                    int minCost = Math.min(cost[j][k],cost[j][i]+cost[i][k]);
                    cost[j][k] = minCost;
                    cost[k][j] = minCost;
                }
            }
    ...
    public List<Integer> findMinHeightTrees2(int n, int[][] edges) {
        ...
        while (!queue.isEmpty()){
            ...
            for (int i = 0; i < size; i++) {
                int cur = queue.poll();
                res.add(cur);
                List<Integer> neignbor = map.get(cur);
                for (Integer integer : neignbor) {
                    degree[integer]--;
                    if(degree[integer]==1){
                        queue.offer(integer);
                    }
